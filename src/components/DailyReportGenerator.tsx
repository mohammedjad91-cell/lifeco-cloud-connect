...
export default function DailyReportGenerator({ department, date }: Props) {
  ...
  const run = async () => {
    ...
    try {
      ...
      if (result.error) {
        toast({ title: "أرجع الذكاء الاصطناعي خطأ", variant: "destructive" });
      } else {
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold neon-text flex items-center gap-2">
          <Sparkles className="w-5 h-5" /> التقرير اليومي الذكي
        </h3>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <FormField
          label="رقم واتساب"
          hint="أدخل رمز الدولة، أرقام فقط. اتركه فارغًا لاختيار محادثة داخل واتساب."
        >
          {(id) => (
        ...
        <FormField
          label="بريد المستلم"
          hint="يفتح Gmail مع نص التقرير معبأً مسبقًا. اتركه فارغًا لاختيار المستلم هناك."
        >
          {(id) => (
            <div className="flex gap-2">
              <Input
      ...
      <FormField
        label="مسودة التقرير"
        hint="أُنشئت بواسطة الذكاء الاصطناعي من قراءات اليوم — عدّلها بحرية قبل التصدير أو المشاركة."
      >
        {(id) => (
          <Textarea
            id={id}
            ...
            rows={16}
            placeholder="اضغط إنشاء لبناء التقرير، ثم شاركه عبر واتساب أو Gmail."
