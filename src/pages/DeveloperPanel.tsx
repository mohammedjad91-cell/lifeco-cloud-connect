...
function ThresholdsSection() {
  ...
  return (
    <div className="space-y-6">
      <Card title="إضافة/تعديل حد تنبيه" icon={Plus}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-2">
          <select value={form.equipment_tag} onChange={(e)=>setForm({...form, equipment_tag:e.target.value})}
            className="bg-slate-800 border border-white/10 rounded-md px-3 text-sm h-10">
      ...
      <Card title={`قائمة حدود التنبيهات (${rows.length})`} icon={AlertCircle}
        action={<Button size="sm" variant="ghost" onClick={load}><RefreshCw className="w-4 h-4"/></Button>}>
            ...
            <table className="w-full text-sm text-right">
              <thead>
                <tr className="border-b border-white/10 text-white/60">
                  ...
                  <th className="p-2">البارامتر</th>
                  <th className="p-2">الأدنى</th>
                  <th className="p-2">الأقصى</th>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="p-2">{r.min_value ?? "—"}</td>
                    <td className="p-2">{r.max_value ?? "—"}</td>
                    <td className="p-2">
